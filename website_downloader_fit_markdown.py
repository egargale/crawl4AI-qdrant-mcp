"""
Website Downloader with LLM Extraction
--------------------------------------
...
class WebsiteDownloader:
    ...
    async def crawl_recursive(self):
            ...
            if not self.llm_api_key:
                ...
                try:
                    ...
                    extraction_strategy = LLMExtractionStrategy(
                        ...
                        and key points. Focus on the most important information and remove noise
                        like navigation, ads, and irrelevant content.""",
                    ...
                    if result.success:
                        ...
                        if self.use_llm_extraction and hasattr(result, 'extracted_content') and result.extracted_content:
                            try:
                                ...
                                self.save_markdown(result.url, result.markdown, extracted_data)
                            except (json.JSONDecodeError, Exception) as e:
                                print(f"Error parsing extracted content: {e}")
                                # Fallback to standard markdown
                                self.save_markdown(result.url, result.markdown)
                        else:
                            # Save standard markdown
                            self.save_markdown(result.url, result.markdown)
        ...
        # Show LLM usage statistics if available
...
USAGE:
    python website_downloader_fit_markdown.py <URL> [OPTIONS]
...
EXAMPLES:
    ...
    python website_downloader_fit_markdown.py https://docs.crawl4ai.com

    # Download with custom output directory and depth
    python website_downloader_fit_markdown.py https://example.com -o my_site -d 2

    # Download with increased concurrency for faster crawling
    python website_downloader_fit_markdown.py https://example.com -c 10

    # Download with different LLM provider
    python website_downloader_fit_markdown.py https://example.com --llm-provider ollama/llama2

    # Download with custom LLM base URL
    python website_downloader_fit_markdown.py https://example.com --llm-base-url https://custom-llm-endpoint.com/v1

    # Download without LLM extraction (standard markdown only)
    python website_downloader_fit_markdown.py https://example.com --no-llm
...
OUTPUT:
    ...
    - *.json files contain the raw extracted data for programmatic access
