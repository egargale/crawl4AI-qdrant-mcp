#!/usr/bin/env python3
"""
Script to search for "fastmcp integration" in the Qdrant manuali collection.
"""
...
async def search_fastmcp_integration():
    """Search for 'fastmcp integration' in the manuali collection."""
    
    # Load environment variables
    load_dotenv()
    
    collection_name = "manuali"
    query_text = "fastmcp integration"
    
    safe_print(f"Searching for '{query_text}' in collection: {collection_name}")
                ...
                if hasattr(point, 'payload') and point.payload:
                    ...
                    if 'document' in point.payload:
                        ...
                        try:
                            ...
                            safe_print(f"Content: {clean_content[:800]}...")
                        except Exception:
                            # Ultimate fallback
                            safe_print(f"Content: {content[:300]}...")
                    # Print source metadata
                    if 'source' in point.payload:
                        safe_print(f"Source: {point.payload['source']}")
                    # Print other metadata
                    other_payload = {k: v for k, v in point.payload.items() if k not in ['document', 'source']}
                    if other_payload:
                        safe_print(f"Metadata: {other_payload}")
        else:
...
def main():
    ...
    safe_print("Searching 'manuali' collection for 'fastmcp integration'...")
    
    try:
        # Run the async search
        success = asyncio.run(search_fastmcp_integration())
